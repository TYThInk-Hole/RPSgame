...
function ERPS_intra(Lsize, reproduction_rate, selection_rate, mobility, intra1, intra2, intra3, intra4, intra5, ext, para, rn)
    ...
    h5open(file_dir, "r+") do f
        ...
        if !haskey(g["NumS"], "$rn")
            create_dataset(g["NumS"], "$rn", Float64, ((1, 5), (-1, 5)), chunk=(1, 5))
        end
    end
...
function parse_command_line_args()
    if length(ARGS) != 15
        ...
        println("Usage: julia ERPS_CS_test.jl Lsize reproduction_rate selection_rate mobility intra1 intra2 intra3_start intra3_end intra3_step intra4 intra5 ext para rn_start rn_end")
        exit(1)
    end
    ...
    return (
        ...
        parse(Float64, ARGS[7]),  # intra3_start
        parse(Float64, ARGS[8]),  # intra3_end
        parse(Float64, ARGS[9]),  # intra3_step
        parse(Float64, ARGS[10]),  # intra4
        parse(Float64, ARGS[11]),  # intra5

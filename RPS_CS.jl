using Random, HDF5, Printf, Statistics, SparseArrays, StatsBase, Base.Threads, Distributed
...
# 필요한 모듈을 모든 워커에 로드
@everywhere using Random, HDF5, Printf, Statistics, SparseArrays, StatsBase

# RPS_intra 함수를 모든 워커에 정의
@everywhere function RPS_intra(Lsize, reproduction_rate, selection_rate, mobility, intra1, intra2, intra3, ext, para, rn)
    # 여기에 RPS_intra 함수의 전체 내용을 복사-붙여넣기 하세요
end

# 병렬 실행
@sync @distributed for rn in rn_start:rn_end
